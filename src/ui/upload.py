...
from src.embedder import (
    ...
    delete_file_chunks, invalidate_metadata_cache
)
...
def render_tab_upload():
    ...
    **Examples:** `HDFC_Life_Q1_FY25.pdf`, `SBI_Life_Q2_FY25.pdf`
    ...
    if uploaded_files:
        ...
        if invalid_files:
            st.error(f"❌ Invalid filename(s) — must follow `{{COMPANY_CODE}}_{{QUARTER}}_{{FY}}.pdf`:\n" + "\n".join(f"  • {n}" for n in invalid_files))
            return
        ...
        if st.button("🚀 Start Ingestion", type="primary"):
            ...
            if processing_mode == "Parallel":
                ...
                while True:
                    ...
                    if processing:
                        ...
                        for job in processing:
                            stage = "Starting..."
                            if job.progress >= 0.9: stage = "Storing embeddings..."
                            elif job.progress >= 0.6: stage = "Generating embeddings..."
                            elif job.progress >= 0.4: stage = "Chunking document..."
                            elif job.progress >= 0.2: stage = "Parsing PDF..."
                            status_lines.append(f"- {job.filename}: {stage} ({int(job.progress * 100)}%)\n")
                        status_lines.append("\n")
                    if pending:
                        status_lines.append(f"**⏳ Waiting ({len(pending)} files):**\n")
                        for job in pending[:3]: status_lines.append(f"- {job.filename}\n")
                        if len(pending) > 3: status_lines.append(f"- ... and {len(pending) - 3} more\n")
                        status_lines.append("\n")
                    completed_jobs = [j for j in jobs.values() if j.status == JobStatus.COMPLETED]
                    if completed_jobs: status_lines.append(f"**✅ Completed ({len(completed_jobs)} files)**\n\n")
                    failed_jobs = [j for j in jobs.values() if j.status == JobStatus.FAILED]
                    if failed_jobs:
                        status_lines.append(f"**❌ Failed ({len(failed_jobs)} files):**\n")
                        for job in failed_jobs: status_lines.append(f"- {job.filename}: {job.error}\n")
                        status_lines.append("\n")
                ...
                for job_id in job_ids:
                    ...
                    if job.status == JobStatus.COMPLETED and job.result: results.append(job.result)
                    elif job.status == JobStatus.FAILED: results.append({"status": "error", "source_file": job.filename, "message": job.error or "Unknown error"})
                worker.clear_completed_jobs()
            else:
                import gc
                st.info(f"📝 Processing {len(temp_paths)} files sequentially (cloud-safe mode)…")
                st.caption("⚠️ On Streamlit Community Cloud, processing large PDFs may take 30–60s each. If the app restarts mid-way, re-upload remaining files — already-indexed files will be skipped.")
                progress_bar = st.progress(0)
                status_placeholder = st.empty()
                for idx, temp_path in enumerate(temp_paths, 1):
                    filename = Path(temp_path).name
                    status_placeholder.markdown(f"### 📊 Processing\n\n**🔄 File {idx}/{len(temp_paths)}:** `{filename}` — please wait, this may take up to 60s…")
                    try:
                        result = ingest_pdf(temp_path, force_reindex=False)
                    except Exception as _exc:
                        result = {"status": "error", "message": str(_exc), "chunks_created": 0, "duration_seconds": 0}
                        logger.exception("[UPLOAD] ingest_pdf failed for %s", filename)
                    results.append(result)
                    gc.collect()
                    progress_bar.progress(idx / len(temp_paths))
                    if result['status'] == 'success': status_placeholder.success(f"✓ {filename}: {result['chunks_created']} chunks · {result['duration_seconds']}s")
                    elif result['status'] == 'skipped': status_placeholder.info(f"⊘ {filename}: Already indexed — skipped")
                    else: status_placeholder.error(f"✗ {filename}: {result.get('message', 'Error')}")
                    time.sleep(0.5)
                status_placeholder.empty()
                progress_bar.empty()
            for temp_path in temp_paths: Path(temp_path).unlink(missing_ok=True)
            st.markdown("---")
            ...
            with col1: st.metric("Success", success_count)
            with col2: st.metric("Skipped", skipped_count)
            with col3: st.metric("Errors", error_count)
            for result in results:
                ...
                elif result['status'] == 'skipped': st.info(f"⊘ {result['source_file']}: Already indexed")
                else: st.error(f"✗ {result['source_file']}: {result['message']}")
            if processing_mode == "Parallel": worker.clear_completed_jobs()
        ...
        if all_data['ids']:
            ...
            for metadata in all_data['metadatas']:
                ...
                if source not in files_data:
                    files_data[source] = {
                        'Company': metadata['company'], 'Quarter': metadata['quarter'], 'FY': metadata['fy'],
                        'Period': metadata['period_label'], 'Chunks': 0, 'Ingested At': metadata.get('ingested_at', 'Unknown')
                    }
            ...
            with col_reindex:
                if st.button("🔄 Re-index Selected File", type="secondary"):
                    ...
                    if os.path.exists(pdf_path):
                        ...
                        else: st.error(f"Re-index failed: {result['message']}")
                        st.rerun()
                    else: st.error(f"PDF not found in {PDF_INPUT_DIR}. Upload the file again.")
