...
def render_tab_ask_question():
    ...
    with st.expander("💡 Quick Commands for Definitions"):
        st.markdown("""
        **Add Page Definition:** `define GWP as L-4`
    ...
    if st.button("🔍 Get Answer", type="primary", use_container_width=True):
        ...
        if company_filter:
            filters["company_code"] = company_filter[0] if len(company_filter) == 1 else {"$in": company_filter}
        ...
        with st.spinner("🤔 Thinking..."):
            try:
                ...
                # Persist answer — survives model-dropdown reruns
                st.session_state["last_answer"] = {
                    "question": question,
                    "result": result,
                    "filters": filters,
                    "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "model_selected": st.session_state.get("selected_model_label", ""),
                }
    ...
    # Always render last answer — stays visible even after model-selector rerun
    _render_last_answer()


def _render_last_answer():
    """Render the most recent answer stored in session_state.
    This function is called unconditionally so the answer persists
    when the user changes model in the sidebar (which triggers a rerun).
    """
    ...
    if confidence == 'high':
        badge_class, badge_text = 'confidence-high', '✓ High Confidence'
    elif confidence == 'medium':
        badge_class, badge_text = 'confidence-medium', '⚠ Medium Confidence'
    else:
        badge_class, badge_text = 'confidence-low', '✗ No Data'
    ...
    # Copy button (JavaScript clipboard)
    answer_json = json.dumps(result['answer'])
    copy_html = f"""
    <div style="margin:10px 0;">
        <button onclick="copyAns()" style="
            background:var(--secondary-background-color,#f0f2f6);
            border:1px solid var(--border-color,#d0d0d0);
            border-radius:4px; padding:8px 16px; cursor:pointer;
            font-size:14px; font-weight:500; color:var(--text-color,#262730);
            display:inline-flex; align-items:center; gap:6px;">
            📋 Copy Answer
        </button>
        <span id="ans-fb" style="margin-left:10px;color:#28a745;font-weight:500;display:none;">
            ✓ Copied!
        </span>
    </div>
    <script>
        function copyAns(){{
            navigator.clipboard.writeText({answer_json}).then(function(){{
                var f=document.getElementById('ans-fb');
                f.style.display='inline';
                setTimeout(function(){{f.style.display='none';}},3000);
            }});
        }}
    </script>"""
